    ...
    try:
        async with httpx.AsyncClient(timeout=30.0) as client:
            ...
            if response.status_code == 200:
                ...
                # Check for API error messages - only if message is a string (not a list of results)
                if isinstance(data, dict) and "message" in data:
                    msg = data.get("message")
                    if isinstance(msg, str):
                        if "insufficient_credits" in msg.lower() or "credit" in msg.lower():
                ...
                # Response should be a list of markets - API returns {"message": [...]}
                markets = data if isinstance(data, list) else data.get("message", data.get("markets", data.get("data", [])))
