...
export default function CityManager({ onCityChange }: Props) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      for (const key of runningCities) {
        ...
        try {
          const status = await getScrapeStatus(city, state);
  ...
  const handleAddCity = async () => {
    ...
    try {
      await createCity(newCity.trim(), newState.trim());
      setNewCity('');
      setNewState('');
      await fetchCities();
      onCityChange?.();
  ...
  const handleDeleteCity = async (city: string, state: string) => {
    if (!confirm(`Delete ${city}, ${state} and all associated data?`)) return;

    try {
      await deleteCity(city, state);
      await fetchCities();
      onCityChange?.();
    } catch (error) {
  ...
  const handleStartScrape = async (city: string, state: string) => {
    const key = `${city}_${state}`;
    try {
      const status = await startScrape(city, state);
      setScrapeStatuses(prev => ({ ...prev, [key]: status }));
    } catch (error) {
  ...
  const getScrapeStatusBadge = (city: string, state: string) => {
    const key = `${city}_${state}`;
                ...
                return (
                  <tr key={c.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      {getScrapeStatusBadge(c.city, c.state)}
                    </td>
                    <td className="px-4 py-3 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => handleStartScrape(c.city, c.state)}
                          disabled={isRunning}
                        ...
                        <button
                          onClick={() => handleDeleteCity(c.city, c.state)}
